...
class App(tk.Tk):
    def __init__(self):
        ...
        self.title("Laser Software v4_Rev6 (Rain Sensor)")
        ...
        self.after(1000, self._poll_rain_sensor)   # Rain sensor polling every 1s
    ...
    def _build_ui(self):
        ...
        self._build_conn_settings_tab(tab_conn)
    ...
    def _build_config_tab(self, parent):
        ...
        ttk.Label(lf, text="rain_api_url").grid(row=2, column=0, sticky="w", padx=6, pady=6)
        ttk.Entry(lf, textvariable=self.rain_api_url_var, width=70).grid(row=2, column=1, sticky="we", padx=6, pady=6)
        ttk.Label(lf, text="(e.g. http://192.168.3.150:8000/api/rain)",
                  foreground="gray").grid(row=2, column=2, padx=6, sticky="w")

        ttk.Label(lf, text="Logs directory").grid(row=3, column=0, sticky="w", padx=6, pady=6)
        ttk.Entry(lf, textvariable=self.log_dir_var, width=70).grid(row=3, column=1, sticky="we", padx=6, pady=6)
        ...
        ttk.Label(lf, text="Warmup lead (min)").grid(row=6, column=0, sticky="w", padx=6, pady=6)
        ttk.Entry(lf, textvariable=self.monday_warmup_lead_min_var, width=12).grid(row=6, column=1, sticky="w", padx=6, pady=6)

        ttk.Label(lf, text="DTEMF ready >= (°C)").grid(row=7, column=0, sticky="w", padx=6, pady=6)
        ttk.Entry(lf, textvariable=self.monday_warmup_threshold_var, width=12).grid(row=7, column=1, sticky="w", padx=6, pady=6)
        ...
        ttk.Button(lf, text="Browse", command=browse_dir).grid(row=3, column=2, padx=6, pady=6)
    ...
    def _load_config_into_ui(self):
        ...
        try:
            ...
            self.rain_api_url = data.get(
                "rain_api_url",
                getattr(self, "rain_api_url", "")
            )
    ...
    def _poll_rain_sensor(self):
        ...
        RAIN_STALE_SEC = 10  # grace period ก่อนแสดง N/A

        def worker():
            data = self._fetch_rain_data(timeout=2.5)
            try:
    ...
    def is_raining_now(self) -> bool:
        """Helper – คืน True ถ้าเซ็นเซอร์รายงานว่ามีฝน"""
