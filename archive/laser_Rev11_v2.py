...
class App(tk.Tk):
    def __init__(self):
        ...
        self.title("Laser Software v5_Rev11")
    ...
    def _rearm_prefire_after_resume(self, idx: int) -> None:
        ...
        try:
            ...
            fire_ms = self._minutes_text_to_ms(v["fire_ms"].get())
            rest_ms = self._minutes_text_to_ms(v["rest_ms"].get())
            nxt = self._next_fire_time(datetime.now(TZ), start_dt, end_dt, fire_ms, rest_ms)
            if nxt is None:
                self._sched_log(idx, "Resume: ไม่มีรอบยิงเหลือในหน้าต่างเวลานี้ → ไม่ตั้ง prefire")
                return
            self._schedule_prefire_api(idx, nxt)
            self._sched_log(idx, f"Resume: re-arm prefire สำหรับรอบ {nxt.strftime('%H:%M:%S')}")
        except Exception as e:
    ...
    def _schedule_postrest_api(self, idx: int) -> None:
        ...
        if not self.roof_auto_sched_var.get():
            return
        if not self._is_program_active(idx):
            self._sched_log(idx, "postrest ไม่ถูกตั้ง: โปรแกรมหยุด/ถูก pause แล้ว")
            return

        def _go():
            # ตรวจซ้ำ ณ เวลาที่ timer ทำงานจริง — โปรแกรมอาจถูกหยุดไปแล้ว
            # และผู้ใช้อาจกำลังคุมหลังคา/ยิง manual อยู่
            if not self._is_program_active(idx):
                self._sched_log(idx, "⛔ postrest ยกเลิก: โปรแกรมหยุด/ถูก pause แล้ว → ไม่ปิดหลังคา")
                return
            if self.roof_auto_sched_var.get():
                self._external_off()
