...
#
# เทสต์: test_roof_safety_v3.py  (pytest test_roof_safety_v3.py -v)
#
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.title("Laser Software v5_Rev11 v3")
    ...
    def resume_program(self, idx: int):
        ...
        # re-arm prefire: ตอน pause เรายกเลิก timer ทิ้งไปแล้ว
        # ถ้าไม่ตั้งใหม่ หลังคาจะไม่เปิดให้รอบยิงถัดไปจนกว่าจะถึง on_rest ครั้งต่อไป
        self._rearm_prefire_after_resume(idx)

    def _rearm_prefire_after_resume(self, idx: int) -> None:
        """
        คำนวณเวลายิงรอบถัดไปแล้วตั้ง prefire ใหม่หลัง resume
        ใช้ compute_next_occurrence เหมือน manager loop เพื่อให้รองรับ
        mode once (ใช้ once_date) / select day / ช่วงข้ามเที่ยงคืน ได้ถูกต้อง
        """
        try:
            v = self.programs[idx]
            if not (v.get("runner") and v["runner"].is_alive()):
                return

            now_dt = datetime.now(TZ)
            s_dt, e_dt = self.compute_next_occurrence(idx, now_dt)
            if not s_dt:
                self._sched_log(idx, "Resume: ไม่มีรอบถัดไป → ไม่ตั้ง prefire")
                return

            fire_ms = self._minutes_text_to_ms(v["fire_ms"].get())
            rest_ms = self._minutes_text_to_ms(v["rest_ms"].get())

            # ถ้าอยู่ในหน้าต่างเวลาแล้ว ให้ align ไปรอบยิงถัดไป (เหมือน manager loop)
            if s_dt <= now_dt < e_dt:
                s_dt = self._next_fire_time(now_dt, s_dt, e_dt, fire_ms, rest_ms)
                if s_dt is None:
                    self._sched_log(idx, "Resume: ไม่มีรอบยิงเหลือในหน้าต่างนี้ → ไม่ตั้ง prefire")
                    return

            self._schedule_prefire_api(idx, s_dt)
            self._sched_log(idx, f"Resume: re-arm prefire สำหรับรอบ {s_dt.strftime('%Y-%m-%d %H:%M:%S')}")
        except Exception as e:
            self._sched_log(idx, f"Resume: re-arm prefire ไม่สำเร็จ: {e}")
    ...
    def _standby_before_roof_close(self, reason: str = "") -> None:
        """
        SAFETY INTERLOCK (ฝั่งปิดหลังคา):
        ห้ามปิดหลังคาขณะเลเซอร์ยังยิงอยู่ — ต้องดับเลเซอร์ก่อนเสมอ
        ...
        try:
            ...
            self.log(f"STANDBY-before-close error ({reason}): {e}")

    def roof_close(self, reason: str = "manual/auto"):
        # ดับเลเซอร์ก่อนปิดหลังคาเสมอ (ป้องกันยิงขณะหลังคาปิด)
        self._standby_before_roof_close(reason)
    ...
    def _schedule_roof_close_if_open(self, reason: str = ""):
        ...
        if state == "ON":
            try:
                ...
                pass
            try:
                # เก็บ handle ไว้เพื่อให้ยกเลิกได้ (stop/pause/ฝนตก/manual)
                self._cancel_delayed_roof_close()
                self._delayed_close_after_id = self.after(
                    5000, lambda: self._delayed_roof_close()
                )
            except Exception as e:
                try:
    ...
    def _cancel_delayed_roof_close(self) -> None:
        """ยกเลิก after() ปิดหลังคาที่ค้างอยู่ (ถ้ามี)"""
        aid = getattr(self, "_delayed_close_after_id", None)
        if aid is not None:
            try:
                self.after_cancel(aid)
            except Exception:
                pass
            self._delayed_close_after_id = None
    ...
    def _is_program_active(self, idx: int) -> bool:
        ...
        ใช้กันไม่ให้ timer ที่ค้างอยู่ไปสั่งเปิด/ปิดหลังคาหลังจากโปรแกรมหยุด/ถูก pause แล้ว
        """
        try:
            if idx < 0 or idx >= len(self.programs):
                return False
    ...
    def _is_program_paused(self, idx: int) -> bool:
        """โปรแกรมถูก pause อยู่หรือไม่ (ฝนตก / ผู้ใช้กด Pause)"""
        try:
            if idx < 0 or idx >= len(self.programs):
                return False
            p = self.programs[idx].get("paused")
            return bool(p is not None and p.is_set())
        except Exception:
            return False
    ...
    def _schedule_prefire_api(self, idx: int, start_dt: datetime) -> None:
        ...
        if not self._is_program_active(idx):
            self._sched_log(idx, "prefire ไม่ถูกตั้ง: โปรแกรมหยุด/ถูก pause แล้ว")
            return
        ...
        def _go():
            ...
            if not self._is_program_active(idx):
                self._sched_log(idx, "⛔ prefire ยกเลิก: โปรแกรมหยุด/ถูก pause แล้ว → ไม่เปิดหลังคา")
                return
            if self.is_raining_now():
    ...
    def _schedule_postrest_api(self, idx: int) -> None:
        ...
        if not self.roof_auto_sched_var.get():
            return
        if self._is_program_paused(idx):
            self._sched_log(idx, "postrest ไม่ถูกตั้ง: โปรแกรมถูก pause อยู่")
            return

        def _go():
            # ตรวจซ้ำ ณ เวลาที่ timer ทำงานจริง
            # หมายเหตุ: "ปิดหลังคา" เป็นทิศทางที่ปลอดภัย และมี interlock ดับเลเซอร์ก่อนแล้ว
            # จึงบล็อกเฉพาะตอนโปรแกรมถูก pause (ฝนตก/ผู้ใช้สั่งพัก) ซึ่งผู้ใช้มักคุมหลังคาเอง
            # ถ้าโปรแกรมจบตามปกติยังต้องปิดหลังคาให้เรียบร้อย
            if self._is_program_paused(idx):
                self._sched_log(idx, "⛔ postrest ยกเลิก: โปรแกรมถูก pause → ไม่ปิดหลังคา")
                return
            if self.roof_auto_sched_var.get():
                self._external_off()
    ...
    def _on_rain_started(self):
        ...
        for idx, v in enumerate(self.programs):
            if v.get("runner") and v["runner"].is_alive():
                v["paused"].set()
                # ❗ ต้องยกเลิก prefire/postrest timer ที่ตั้งค้างไว้ด้วย
                # ไม่งั้น timer จะทำงานทีหลัง แล้วสั่งเปิด/ปิดหลังคาเองทั้งที่โปรแกรมหยุดแล้ว
                try:
                    self._cancel_api_timers_for(idx)
                except Exception:
                    pass
                self._sched_log(idx, "⛔ Paused by Rain Sensor (roof timers cancelled)")
                try:
    ...
    def _show_rain_popup(self, now_txt: str):
        ...
        msg = (
            ...
            "All scheduled programs have been PAUSED.\n"
