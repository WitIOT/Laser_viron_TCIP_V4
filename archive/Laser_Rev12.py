...
import ttkbootstrap as tb   # Rev12: modern theme (flatly) + bootstyle colours
...
class App(tb.Window):
    def __init__(self):
        super().__init__(themename="flatly")   # Rev12: modern flat theme
        ...
        # Rev12: เปิด/ปิดทั้งระบบหลังคา และ limit sensor แยกกัน (ตั้งค่าใน Settings)
        self.roof_enabled = True
        ...
        self.title("Laser Software v5_Rev12")
        ...
        # Rev12: Roof / Limit enable toggles (ตั้งค่าใน Settings tab)
        self.roof_enabled_var      = tk.BooleanVar(value=self.roof_enabled)
        self.limit_enabled_var     = tk.BooleanVar(value=self.limit_enabled)
        ...
        # Rev12: apply roof/limit enable state to Main buttons after UI + config loaded
        try:
            self._apply_roof_enabled(bool(getattr(self, "roof_enabled", True)))
            self._apply_limit_enabled(bool(getattr(self, "limit_enabled", True)))
        except Exception:
            pass
    ...
    def _build_ui(self):
        ...
        # ============================================================== #
        #  Rev12 — Main tab: command-bar + card layout (ตาม mockup)        #
        # ============================================================== #
        main_inner = self._make_scrollable_tab(tab_main)
        root = ttk.Frame(main_inner); root.pack(fill=tk.BOTH, expand=True, padx=12, pady=10)

        def _equal_row(parent, n, uniform):
            r = ttk.Frame(parent); r.pack(fill=tk.X, pady=(0, 10))
            for i in range(n):
                r.columnconfigure(i, weight=1, uniform=uniform)
            return r

        # ---------- 1) Status command bar ----------
        cmdbar = ttk.LabelFrame(root, text="Status", padding=8)
        cmdbar.pack(fill=tk.X, pady=(0, 10))

        def _status_item(col, caption):
            cell = ttk.Frame(cmdbar); cell.grid(row=0, column=col, sticky="w", padx=10, pady=2)
            ttk.Label(cell, text=caption, font=("Segoe UI", 8), foreground="gray").pack(anchor="w")
            return cell

        _c = _status_item(0, "Connection")
        self.conn_status = ttk.Label(_c, text="Disconnected", foreground="red",
                                     font=("Segoe UI", 10, "bold"))
        self.conn_status.pack(anchor="w")

        _c = _status_item(1, "Laser")
        self.laser_status_var = tk.StringVar(value="Laser: -")
        ttk.Label(_c, textvariable=self.laser_status_var, foreground="blue",
                  font=("Segoe UI", 10, "bold")).pack(anchor="w")

        _c = _status_item(2, "Rain")
        ttk.Label(_c, textvariable=self.rain_status_var,
                  font=("Segoe UI", 10, "bold")).pack(anchor="w")

        _c = _status_item(3, "Roof")
        self.roof_status_var = tk.StringVar(value="N/A")
        ttk.Label(_c, textvariable=self.roof_status_var,
                  font=("Segoe UI", 10, "bold")).pack(anchor="w")

        _c = _status_item(4, "Time")
        self.clock_var = tk.StringVar(value="Time: -")
        ttk.Label(_c, textvariable=self.clock_var,
                  font=("Segoe UI", 10, "bold")).pack(anchor="w")

        cmdbar.columnconfigure(5, weight=1)
        conn_btns = ttk.Frame(cmdbar); conn_btns.grid(row=0, column=5, sticky="e", padx=4)
        btn_connect = tb.Button(conn_btns, text="Connect", command=self.connect, bootstyle="success")
        btn_connect.pack(side=tk.LEFT, padx=3)
        btn_disconnect = tb.Button(conn_btns, text="Disconnect", command=self.disconnect, bootstyle="secondary")
        btn_disconnect.pack(side=tk.LEFT, padx=3)
        btn_tutorial = tb.Button(conn_btns, text="Tutorial", command=self._start_tutorial, bootstyle="info-outline")
        btn_tutorial.pack(side=tk.LEFT, padx=3)
        self._ui_refs["connect_btn"] = btn_connect
        self._ui_refs["disconnect_btn"] = btn_disconnect

        # ---------- 2) Manual control — big buttons ----------
        man = ttk.LabelFrame(root, text="Manual Control", padding=8)
        man.pack(fill=tk.X, pady=(0, 10))
        self._ui_refs["manual_frame"] = man
        for i in range(3):
            man.columnconfigure(i, weight=1, uniform="man")
        btn_fire = tb.Button(man, text="FIRE", command=self.cmd_fire, bootstyle="danger", padding=(10, 18))
        btn_fire.grid(row=0, column=0, sticky="nswe", padx=4, pady=2)
        btn_standby = tb.Button(man, text="STANDBY", command=self.cmd_standby, bootstyle="warning", padding=(10, 18))
        btn_standby.grid(row=0, column=1, sticky="nswe", padx=4, pady=2)
        btn_stop = tb.Button(man, text="STOP", command=self.cmd_stop, bootstyle="secondary", padding=(10, 18))
        btn_stop.grid(row=0, column=2, sticky="nswe", padx=4, pady=2)
        self._ui_refs["fire_btn"] = btn_fire
        self._ui_refs["standby_btn"] = btn_standby
        self._ui_refs["stop_btn"] = btn_stop

        # ---------- 3) Settings row: Laser settings / Temp control / Sliding roof ----------
        row3 = _equal_row(root, 3, "set")

        # 3a) Laser settings
        setting = ttk.LabelFrame(row3, text="Laser Settings", padding=8)
        setting.grid(row=0, column=0, sticky="nswe", padx=(0, 5))
        ...
        qs_entry = ttk.Entry(f_qs, textvariable=self.qsdelay_var, width=8); qs_entry.pack(side=tk.LEFT)
        self._ui_refs["qs_entry"] = qs_entry
        ttk.Button(f_qs, text="Set", command=self.apply_qsdelay, width=5).pack(side=tk.LEFT, padx=4)
        self.qsdelay_live_var = tk.StringVar(value="-")
        ttk.Label(f_qs, textvariable=self.qsdelay_live_var, width=5, foreground="gray").pack(side=tk.LEFT)
        ...
        fr_entry = ttk.Entry(f_df, textvariable=self.freq_var, width=8); fr_entry.pack(side=tk.LEFT)
        self._ui_refs["df_entry"] = fr_entry
        ttk.Button(f_df, text="Set", command=self.apply_dfreq, width=5).pack(side=tk.LEFT, padx=4)
        self.dfreq_live_var = tk.StringVar(value="-")
        ttk.Label(f_df, textvariable=self.dfreq_live_var, width=5, foreground="gray").pack(side=tk.LEFT)
        fr_entry.bind("<Return>", lambda _: self.apply_dfreq())
        ttk.Label(setting, text="QSDELAY 0–400 · Freq 1–22", foreground="gray",
                  font=("Segoe UI", 8)).pack(anchor="w", padx=5, pady=(2, 0))
        tb.Button(setting, text="Save Settings", command=self.save_config,
                  bootstyle="info").pack(anchor="e", padx=5, pady=6)

        # 3b) Temp control
        tempf = ttk.LabelFrame(row3, text="Temp Control", padding=8)
        tempf.grid(row=0, column=1, sticky="nswe", padx=5)
        self._ui_refs["temp_frame"] = tempf
        temp_enable = tb.Checkbutton(tempf, text="Enable", variable=self.temp_ctl_enabled,
                                     bootstyle="round-toggle")
        temp_enable.grid(row=0, column=0, columnspan=2, padx=5, pady=5, sticky="w")
        ttk.Label(tempf, text="Max LTEMF (°C):").grid(row=1, column=0, padx=5, pady=4, sticky="e")
        temp_max = ttk.Entry(tempf, textvariable=self.max_temp_var, width=8)
        temp_max.grid(row=1, column=1, padx=5, pady=4, sticky="w")
        ttk.Label(tempf, text="Max DTEMF (°C):").grid(row=2, column=0, padx=5, pady=4, sticky="e")
        temp_dmax = ttk.Entry(tempf, textvariable=self.max_dtemf_var, width=8)
        temp_dmax.grid(row=2, column=1, padx=5, pady=4, sticky="w")
        ttk.Label(tempf, text="เกินค่า → STANDBY อัตโนมัติ", foreground="gray",
                  font=("Segoe UI", 8)).grid(row=3, column=0, columnspan=2, sticky="w", padx=5, pady=(2, 0))
        self._ui_refs["temp_enable"] = temp_enable
        self._ui_refs["temp_max"]    = temp_max
        self._ui_refs["temp_dmax"]   = temp_dmax

        # 3c) Sliding roof (enable toggle ย้ายไป Settings tab แล้ว)
        roof_group = ttk.LabelFrame(row3, text="Sliding Roof", padding=8)
        roof_group.grid(row=0, column=2, sticky="nswe", padx=(5, 0))
        self._ui_refs["roof_frame"] = roof_group
        rbtns = ttk.Frame(roof_group); rbtns.pack(fill=tk.X, pady=(0, 6))
        btn_roof_open = tb.Button(rbtns, text="Open", command=self.roof_open, bootstyle="info")
        btn_roof_open.pack(side=tk.LEFT, expand=True, fill=tk.X, padx=2)
        btn_roof_close = tb.Button(rbtns, text="Close", command=self.roof_close, bootstyle="secondary")
        btn_roof_close.pack(side=tk.LEFT, expand=True, fill=tk.X, padx=2)
        rstat = ttk.Frame(roof_group); rstat.pack(fill=tk.X, pady=2)
        ttk.Label(rstat, text="Status:").pack(side=tk.LEFT, padx=(2, 4))
        self.roof_status_lbl = ttk.Label(rstat, textvariable=self.roof_status_var,
                                         font=("Segoe UI", 9, "bold"))
        self.roof_status_lbl.pack(side=tk.LEFT)
        ...
        self.roof_auto_sched_cb = tb.Checkbutton(
            roof_group, text="Auto open (T-15s) / close (+3s)",
            variable=self.roof_auto_sched_var, bootstyle="round-toggle")
        self.roof_auto_sched_cb.pack(anchor="w", pady=(6, 0))
        self._update_roof_auto_label()
        self._ui_refs["roof_auto_cb"] = self.roof_auto_sched_cb

        # ---------- 4) Telemetry row: Laser temps / Rain sensor ----------
        row4 = _equal_row(root, 2, "tel")

        tele = ttk.LabelFrame(row4, text="Laser Temperatures", padding=8)
        tele.grid(row=0, column=0, sticky="nswe", padx=(0, 5))
        self._ui_refs["tele_frame"] = tele
        tcells = ttk.Frame(tele); tcells.pack(fill=tk.X)
        for i in range(2):
            tcells.columnconfigure(i, weight=1, uniform="tcell")
        d_cell = ttk.Frame(tcells, relief="groove", borderwidth=1, padding=8)
        d_cell.grid(row=0, column=0, sticky="nswe", padx=3, pady=3)
        ttk.Label(d_cell, text="DTEMF", foreground="gray").pack(anchor="w")
        self.lbl_dtemf = ttk.Label(d_cell, text="-", font=("Segoe UI", 22, "bold"))
        self.lbl_dtemf.pack(anchor="w")
        ttk.Label(d_cell, text="max", foreground="gray", font=("Segoe UI", 8)).pack(side=tk.LEFT)
        ttk.Label(d_cell, textvariable=self.max_dtemf_var, foreground="gray",
                  font=("Segoe UI", 8)).pack(side=tk.LEFT, padx=2)
        l_cell = ttk.Frame(tcells, relief="groove", borderwidth=1, padding=8)
        l_cell.grid(row=0, column=1, sticky="nswe", padx=3, pady=3)
        ttk.Label(l_cell, text="LTEMF", foreground="gray").pack(anchor="w")
        self.lbl_ltemf = ttk.Label(l_cell, text="-", font=("Segoe UI", 22, "bold"))
        self.lbl_ltemf.pack(anchor="w")
        ttk.Label(l_cell, text="max", foreground="gray", font=("Segoe UI", 8)).pack(side=tk.LEFT)
        ttk.Label(l_cell, textvariable=self.max_temp_var, foreground="gray",
                  font=("Segoe UI", 8)).pack(side=tk.LEFT, padx=2)
        csvrow = ttk.Frame(tele); csvrow.pack(fill=tk.X, pady=(8, 0))
        self.record_var = tk.BooleanVar(value=False)
        tb.Checkbutton(csvrow, text="Save CSV", variable=self.record_var,
                       command=self._toggle_telemetry, bootstyle="round-toggle").pack(side=tk.LEFT)
        self.csv_name_var = tk.StringVar(value=self._default_csv_name())
        ttk.Entry(csvrow, textvariable=self.csv_name_var).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=6)

        rain_frm = ttk.LabelFrame(row4, text="Rain Sensor", padding=8)
        rain_frm.grid(row=0, column=1, sticky="nswe", padx=(5, 0))
        self._ui_refs["rain_frame"] = rain_frm
        rtop = ttk.Frame(rain_frm); rtop.pack(fill=tk.X)
        ttk.Label(rtop, text="Status:").pack(side=tk.LEFT, padx=(0, 4))
        self.rain_status_lbl = ttk.Label(rtop, textvariable=self.rain_status_var,
                                         font=("Segoe UI", 10, "bold"), foreground="gray")
        self.rain_status_lbl.pack(side=tk.LEFT)
        rmetrics = ttk.Frame(rain_frm); rmetrics.pack(fill=tk.X, pady=6)
        for i in range(2):
            rmetrics.columnconfigure(i, weight=1, uniform="rm")
        ri = ttk.Frame(rmetrics, relief="groove", borderwidth=1, padding=6)
        ri.grid(row=0, column=0, sticky="nswe", padx=3)
        ttk.Label(ri, text="Intensity", foreground="gray").pack(anchor="w")
        ttk.Label(ri, textvariable=self.rain_intensity_var, font=("Segoe UI", 16, "bold")).pack(anchor="w")
        rt = ttk.Frame(rmetrics, relief="groove", borderwidth=1, padding=6)
        rt.grid(row=0, column=1, sticky="nswe", padx=3)
        ttk.Label(rt, text="Total", foreground="gray").pack(anchor="w")
        ttk.Label(rt, textvariable=self.rain_total_var, font=("Segoe UI", 16, "bold")).pack(anchor="w")
        self.rain_ts_var = tk.StringVar(value="-")
        ttk.Label(rain_frm, textvariable=self.rain_ts_var, foreground="gray",
                  font=("Segoe UI", 8)).pack(anchor="e")

        # ---------- 5) Sensor row: Indoor / Outdoor (Temp = RH) ----------
        row5 = _equal_row(root, 2, "sen")
        self._ui_refs["sensor_frame"] = row5

        def _sensor_panel(parent, title, t_var, h_var, d_var, col):
            lf = ttk.LabelFrame(parent, text=title, padding=8)
            lf.grid(row=0, column=col, sticky="nswe", padx=(0, 5) if col == 0 else (5, 0))
            big = ttk.Frame(lf); big.pack(fill=tk.X)
            for i in range(2):
                big.columnconfigure(i, weight=1, uniform="sc")
            tc = ttk.Frame(big, relief="groove", borderwidth=1, padding=6)
            tc.grid(row=0, column=0, sticky="nswe", padx=3)
            ttk.Label(tc, text="Temp (°C)", foreground="gray").pack(anchor="w")
            ttk.Label(tc, textvariable=t_var, font=("Segoe UI", 18, "bold")).pack(anchor="w")
            hc = ttk.Frame(big, relief="groove", borderwidth=1, padding=6)
            hc.grid(row=0, column=1, sticky="nswe", padx=3)
            ttk.Label(hc, text="Humidity (%)", foreground="gray").pack(anchor="w")
            ttk.Label(hc, textvariable=h_var, font=("Segoe UI", 18, "bold")).pack(anchor="w")
            drow = ttk.Frame(lf); drow.pack(fill=tk.X, pady=(6, 0))
            ttk.Label(drow, text="Dew point:", foreground="gray").pack(side=tk.LEFT)
            ttk.Label(drow, textvariable=d_var, font=("Segoe UI", 10, "bold")).pack(side=tk.LEFT, padx=4)
            ttk.Label(drow, text="°C", foreground="gray").pack(side=tk.LEFT)

        _sensor_panel(row5, "Indoor Sensor", self.sensor_in_temp_var,
                      self.sensor_in_humi_var, self.sensor_in_dew_var, 0)
        _sensor_panel(row5, "Outdoor Sensor", self.sensor_out_temp_var,
                      self.sensor_out_humi_var, self.sensor_out_dew_var, 1)
        ttk.Label(row5, textvariable=self.sensor_ts_var, foreground="gray",
                  font=("Segoe UI", 8)).grid(row=1, column=0, columnspan=2, sticky="w", pady=(2, 0))

        # ---------- 6) Scheduled programs ----------
        prog_box = ttk.LabelFrame(root, text="Scheduled Programs", padding=8)
        ...
        btn_add_prog = tb.Button(toolbar, text="+ Add Program", command=self.add_program, bootstyle="info")
        btn_add_prog.pack(side=tk.LEFT, padx=4)
        btn_start_all = tb.Button(toolbar, text="Start All", command=self.start_all, bootstyle="success")
        btn_start_all.pack(side=tk.LEFT, padx=4)
        btn_stop_all = tb.Button(toolbar, text="Stop All", command=self.stop_all_programs, bootstyle="secondary")
        btn_stop_all.pack(side=tk.LEFT, padx=4)
        btn_remove_all = tb.Button(toolbar, text="Remove All", command=self.remove_all_programs, bootstyle="danger-outline")
        ...
        self.prog_nb = ttk.Notebook(prog_box); self.prog_nb.pack(fill=tk.BOTH, expand=True, pady=(4, 0))

        # ---------- 7) Charts + Logs ----------
        vis = ttk.Panedwindow(root, orient=tk.HORIZONTAL); vis.pack(fill=tk.BOTH, expand=True, pady=(0, 4))
        ...
        nb2 = ttk.Notebook(logs_container); nb2.pack(fill=tk.BOTH, expand=True, padx=4, pady=4)

        tab_all = ttk.Frame(nb2); nb2.add(tab_all, text="All except Schedule")
        ttk.Button(tab_all, text="Clear", command=self.clear_terminal).pack(anchor="ne", padx=6, pady=4)
        self.log_text = tk.Text(tab_all, height=12); self.log_text.pack(fill=tk.BOTH, expand=True)

        tab_sched = ttk.Frame(nb2); nb2.add(tab_sched, text="Schedule Logs")
        ttk.Button(tab_sched, text="Clear", command=self.clear_sched_terminal).pack(anchor="ne", padx=6, pady=4)
        self.sched_log_text = tk.Text(tab_sched, height=12); self.sched_log_text.pack(fill=tk.BOTH, expand=True)

        # ---------- Terminal Tab ----------
        tab_term = ttk.Frame(nb2); nb2.add(tab_term, text="Terminal")
    ...
    def _build_config_tab(self, parent):
        ...
        # Rev12: Enable roof toggle (ย้ายมาจากหน้า Main)
        tb.Checkbutton(
            roof_lf, text="Enable roof system",
            variable=self.roof_enabled_var,
            command=lambda: self._apply_roof_enabled(bool(self.roof_enabled_var.get())),
            bootstyle="round-toggle",
        ).grid(row=0, column=0, columnspan=2, sticky="w", padx=6, pady=(8, 4))

        ttk.Label(roof_lf, text="roof_api_base").grid(row=1, column=0, sticky="w", padx=6, pady=6)
        ttk.Entry(roof_lf, textvariable=self.roof_api_base_var, width=55).grid(row=1, column=1, sticky="we", padx=6, pady=6)

        # Rev12: Limit sub-group with its own enable toggle
        limit_lf = ttk.LabelFrame(roof_lf, text="Limit (roof position sensor)")
        limit_lf.grid(row=2, column=0, columnspan=2, sticky="we", padx=6, pady=(2, 6))
        limit_lf.columnconfigure(1, weight=1)
        tb.Checkbutton(
            limit_lf, text="Enable limit",
            variable=self.limit_enabled_var,
            command=lambda: self._apply_limit_enabled(bool(self.limit_enabled_var.get())),
            bootstyle="round-toggle",
        ).grid(row=0, column=0, columnspan=2, sticky="w", padx=6, pady=(6, 2))
        ttk.Label(limit_lf, text="limit_api_url").grid(row=1, column=0, sticky="w", padx=6, pady=6)
        ttk.Entry(limit_lf, textvariable=self.limit_api_url_var, width=50).grid(row=1, column=1, sticky="we", padx=6, pady=6)
        ttk.Label(limit_lf,
                  text="ปิด limit: ข้ามการยืนยันตำแหน่งหลังคา (Open/Close ยังสั่งได้, Safety Fire ถูกข้าม)",
                  foreground="gray").grid(row=2, column=0, columnspan=2, sticky="w", padx=6, pady=(0, 6))
        ...
        ttk.Label(roof_lf, text="Pre-open lead (sec)").grid(row=3, column=0, sticky="w", padx=6, pady=6)
        ttk.Entry(roof_lf, textvariable=self.prefire_open_sec_var, width=12).grid(row=3, column=1, sticky="w", padx=6, pady=6)

        ttk.Label(roof_lf, text="Post-close delay (sec)").grid(row=4, column=0, sticky="w", padx=6, pady=6)
        ttk.Entry(roof_lf, textvariable=self.postrest_close_sec_var, width=12).grid(row=4, column=1, sticky="w", padx=6, pady=6)
        ...
        ttk.Label(roof_lf,
                  text="⚠ ปิด Roof หรือ Limit แล้ว FIRE จะไม่มีการป้องกันเรื่องหลังคา",
                  foreground="#cc6600").grid(row=6, column=0, columnspan=2, sticky="w", padx=6, pady=(0, 6))
        ...
        tb.Checkbutton(sensor_lf, text="Enable Temp & RH Sensor",
    ...
    def _apply_config_tab(self):
        ...
        try:
            ...
            # Rev12: apply roof/limit enable (ใช้ method เพื่อ sync ปุ่ม/สถานะด้วย)
            self._apply_roof_enabled(bool(self.roof_enabled_var.get()))
            self._apply_limit_enabled(bool(self.limit_enabled_var.get()))
    ...
    def save_config(self):
        try:
            data = {
                ...
                "roof_enabled":      bool(getattr(self, "roof_enabled", True)),
                "limit_enabled":     bool(getattr(self, "limit_enabled", True)),
    ...
    def _load_config_into_ui(self):
        ...
        try:
            ...
            self.roof_enabled      = bool(data.get("roof_enabled", getattr(self, "roof_enabled", True)))
            self.limit_enabled     = bool(data.get("limit_enabled", getattr(self, "limit_enabled", True)))
            ...
            if hasattr(self, "roof_enabled_var"):
                self.roof_enabled_var.set(self.roof_enabled)
    ...
    def roof_open(self):
        if not getattr(self, "roof_enabled", True):
            self.log("⛔ Roof OPEN blocked: roof system disabled (see Settings).")
            return
        if self.is_raining_now():
            def _warn():
                try:
    ...
    def _schedule_prefire_api(self, idx: int, start_dt: datetime) -> None:
        ...
        if not self.roof_auto_sched_var.get() or not getattr(self, "roof_enabled", True):
            return
    ...
    def _apply_roof_enabled(self, enabled: bool):
        """Rev12: เปิด/ปิดทั้งระบบหลังคา — เรียกจาก main thread เท่านั้น
        เมื่อปิด: disable ปุ่ม Open/Close/auto บนหน้า Main, หยุดเรียก roof API,
        ข้าม auto open/close, ข้าม Safety Fire และไม่สั่งปิดหลังคาตอนฝนตก"""
        self.roof_enabled = bool(enabled)
        try:
            self.roof_enabled_var.set(self.roof_enabled)
        except Exception:
            pass
        state = "normal" if enabled else "disabled"
        for key in ("roof_open_btn", "roof_close_btn", "roof_auto_cb"):
            w = self._ui_refs.get(key)
            if w is not None:
                try:
        ...
        if not enabled:
            try:
                self.roof_status_var.set("Disabled")
            except Exception:
                pass
        try:
            self.log(f"Roof system = {'ENABLED' if enabled else 'DISABLED'}")
        except Exception:
            pass

    def _apply_limit_enabled(self, enabled: bool):
        """Rev12: เปิด/ปิด limit (roof position sensor) แยกจาก roof
        เมื่อปิด: ไม่อ่าน limit_api_url, ไม่ยืนยันตำแหน่งหลังคา, ข้าม Safety Fire"""
        ...
        if not enabled:
            self._roof_state_cached = "N/A"
            try:
                self.roof_status_var.set("Limit off")
            except Exception:
                pass
        try:
            self.log(f"Limit sensor = {'ENABLED' if enabled else 'DISABLED'}")
        except Exception:
            pass
    ...
    def _on_rain_started(self):
        ...
        # 2. ปิดหลังคาถ้าเปิดอยู่ (Rev12: เคารพ Roof disabled — ไม่สั่งปิด)
        if getattr(self, "roof_enabled", True):
            try:
                state = self._get_roof_status_cached()
                if state == "ON":
                    self.roof_close()
            ...
            self.log("🌧 Rain: roof system disabled — skip roof close")
    ...
    def _show_rain_popup(self, now_txt: str):
        ...
        # Rev12: redesigned card popup (English) — header + safety checklist + countdown
        HEADER = "#1b6ec2"; CARD = "#ffffff"; TXT = "#1f2d3d"
        MUTED = "#6c7a89"; OK_GREEN = "#1aa179"; TRACK = "#e6eaf0"
        ...
        win.geometry("440x336+120+120")

        # ---- header bar ----
        header = tk.Frame(win, bg=HEADER, padx=16, pady=13)
        header.pack(fill="x")
        tk.Label(header, text="🌧", font=("Segoe UI", 20), fg="white", bg=HEADER
                 ).pack(side="left", padx=(0, 10))
        htext = tk.Frame(header, bg=HEADER); htext.pack(side="left", anchor="w")
        tk.Label(htext, text="Rain detected", font=("Segoe UI", 14, "bold"),
                 fg="white", bg=HEADER).pack(anchor="w")
        tk.Label(htext, text="Safety actions taken automatically",
                 font=("Segoe UI", 8), fg="#d6e6f7", bg=HEADER).pack(anchor="w")

        # ---- body ----
        body = tk.Frame(win, bg=CARD, padx=18, pady=14)
        body.pack(fill="both", expand=True)

        roof_line = ("Roof closed (if it was open)"
                     if getattr(self, "roof_enabled", True)
                     else "Roof control disabled — not closed")
        for txt in ("All scheduled programs PAUSED", roof_line, "Laser set to STANDBY"):
            row = tk.Frame(body, bg=CARD); row.pack(fill="x", pady=2, anchor="w")
            tk.Label(row, text="✓", font=("Segoe UI", 11, "bold"),
                     fg=OK_GREEN, bg=CARD).pack(side="left", padx=(0, 8))
            tk.Label(row, text=txt, font=("Segoe UI", 10), fg=TXT, bg=CARD).pack(side="left")

        tk.Label(body, text=f"Detected at:  {now_txt} (UTC+7)",
                 font=("Consolas", 9), fg=MUTED, bg=CARD).pack(anchor="w", pady=(10, 6))

        tk.Label(body,
                 text=("Programs will stay paused while it is raining.\n"
                       "Press Resume manually after the rain stops."),
                 font=("Segoe UI", 9), fg=MUTED, bg=CARD, justify="left"
                 ).pack(anchor="w")

        _AUTODISMISS = 60  # seconds
        bottom = tk.Frame(body, bg=CARD); bottom.pack(fill="x", side="bottom", pady=(14, 0))
        countdown_var = tk.StringVar(value=f"Auto-close in {_AUTODISMISS}s")
        tk.Label(bottom, textvariable=countdown_var, font=("Segoe UI", 8),
        ...
        tk.Button(bottom, text="OK", width=10, command=_close, cursor="hand2",
                  bg=HEADER, fg="white", activebackground="#155a9c",
                  activeforeground="white", relief="flat", bd=0,
                  font=("Segoe UI", 10)).pack(side="right")

        # ---- bottom progress bar (shrinks as countdown runs) ----
        track = tk.Frame(win, bg=TRACK, height=3); track.pack(fill="x", side="bottom")
        bar = tk.Frame(track, bg=HEADER, height=3); bar.place(relwidth=1.0, relheight=1.0)

        # countdown tick — runs via after(), does not block the event loop
        ...
        def _tick():
            try:
                ...
                try:
                    bar.place_configure(relwidth=max(0.0, remaining[0] / _AUTODISMISS))
                except Exception:
                    pass
    ...
    def _poll_roof_status(self):
        ...
        # Rev12: ปิด Roof หรือ Limit → ไม่เรียก limit API, ถือว่า N/A
        if not getattr(self, "roof_enabled", True) or not getattr(self, "limit_enabled", True):
            self._roof_state_cached = "N/A"
            self._roof_state_ts = time.monotonic()
            self.after(3000, self._poll_roof_status)
            return
    ...
    def _is_safety_fire_enabled(self) -> bool:
        ...
        Rev12: ถ้าปิด Roof หรือ Limit → ข้าม Safety Fire (ยิงได้เลย ไม่เช็คหลังคา)"""
        if not getattr(self, "roof_enabled", True) or not getattr(self, "limit_enabled", True):
            return False
        try:
            if hasattr(self, "safety_fire_enabled_var"):
