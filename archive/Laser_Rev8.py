...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        rain_frm.grid(row=0, column=2, sticky="nwe", padx=5, pady=5)
        ...
        sensor_frm.grid(row=1, column=2, sticky="nwe", padx=5, pady=5)
        ...
        prog_box.grid(row=2, column=1, columnspan=2, sticky="nwe", padx=5, pady=5)
        ...
        root.rowconfigure(3, weight=1)  # Realtime Charts row expands
    ...
    def _build_config_tab(self, parent):
        ...
        ttk.Label(lf, text="rain_api_url").grid(row=3, column=0, sticky="w", padx=6, pady=6)
        ttk.Entry(lf, textvariable=self.rain_api_url_var, width=70).grid(row=3, column=1, sticky="we", padx=6, pady=6)
        ttk.Label(lf, text="(e.g. http://192.168.3.150:8000/api/rain)",
                  foreground="gray").grid(row=3, column=2, padx=6, sticky="w")

        # Rain Sensor timing
        rain_timing = ttk.LabelFrame(lf, text="Rain Sensor Timing")
        rain_timing.grid(row=4, column=0, columnspan=3, sticky="we", padx=6, pady=(4, 2))
        rain_timing.columnconfigure(2, weight=1)  # hint label column stretches
        ...
        # ---- Temp & RH Sensor Settings ----
        sensor_lf = ttk.LabelFrame(lf, text="Temp & RH Sensor Settings")
        sensor_lf.grid(row=5, column=0, columnspan=3, sticky="we", padx=6, pady=(4, 2))
        ...
        ttk.Entry(sensor_lf, textvariable=self.sensor_api_url_var, width=60).grid(row=1, column=1, sticky="we", padx=4, pady=5)
        ...
        ttk.Label(lf, text="Logs directory").grid(row=6, column=0, sticky="w", padx=6, pady=6)
        ttk.Entry(lf, textvariable=self.log_dir_var, width=70).grid(row=6, column=1, sticky="we", padx=6, pady=6)
        ...
        ).grid(row=7, column=0, columnspan=2, sticky="w", padx=6, pady=6)

        ttk.Checkbutton(
            lf,
            text="Enable Monday STANDBY warmup logic",
            variable=self.monday_warmup_enabled_var
        ).grid(row=8, column=0, columnspan=2, sticky="w", padx=6, pady=6)

        ttk.Label(lf, text="Warmup lead (min)").grid(row=9, column=0, sticky="w", padx=6, pady=6)
        ttk.Entry(lf, textvariable=self.monday_warmup_lead_min_var, width=12).grid(row=9, column=1, sticky="w", padx=6, pady=6)

        ttk.Label(lf, text="DTEMF ready >= (°C)").grid(row=10, column=0, sticky="w", padx=6, pady=6)
        ttk.Entry(lf, textvariable=self.monday_warmup_threshold_var, width=12).grid(row=10, column=1, sticky="w", padx=6, pady=6)
        ...
        ttk.Button(lf, text="Browse", command=browse_dir).grid(row=6, column=2, padx=6, pady=6)
    ...
    def add_program(self, init_data: dict | None = None):
        ...
        name_entry = ttk.Entry(row0, textvariable=pv["name"], width=28)
        ...
        pv["fire_entry"] = ttk.Entry(row1, textvariable=pv["fire_ms"], width=10)
        pv["fire_entry"].pack(side=tk.LEFT, padx=4)
        ttk.Label(row1, text="Rest (min)").pack(side=tk.LEFT)
        pv["rest_entry"] = ttk.Entry(row1, textvariable=pv["rest_ms"], width=10)
